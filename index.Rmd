...
```
The Seattle SPD data contains `r summary_table$columns` columns with `r summary_table$rows` rows of data. It is a very large dataset that contains almost every police incident starting in 2008. It breaks down Offenses into `r summary_table$offense_parent_types` parent types, with a total of `r summary_table$offense_types` unique offense types. Some interesting stats which we figured out from this dataset can include Safest/worst neighborhoods, and common offense types. We found that the average crimes per neighborhood throughout the city was `r format(summary_table$avg_crimes_per_neighborhood, scientific=FALSE)` per neighborhood overall. 
...
average delay between crime occurrence to report filed in days (from 2008 - present). This unveils the correlation between crime types (i.e. assault, etc.) & how long it takes people to submit reports. Total crime occurrences were included to give perspective on the frequency of a specific crime. The rows are ordered by descending report delay time. Out of the top 8 longest delay times, 6 of them were crimes involving sex. Understandably, this may occur because the victim and/or those knowing of the offense may need time to process/collect/feel safe enough to report the event. For instance, while incest only had 42 occurrences since 2008, the average report delay time was nearly 3 years.
